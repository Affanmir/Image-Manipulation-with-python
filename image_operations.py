...
class MyImage:
    ...
    def open(self, path: str,pointer =False):
        ...
        myimg: MyImage = MyImage(img.size,pointer=False)
...
def rotations(src: MyImage) -> MyImage:
    ...
    copy = MyImage((x*2, y*2),src.pointer) #we make a copy which is twice the size of the original
...
def horizontal_vertical_bonus(src: MyImage) -> MyImage:
    x , y = src.size
    copy = MyImage((x,y),src.pointer)
    for i in range(x):
        for j in range(y):
        ...
        for j in range(y):
            ...
            for maski in range(n): # for every pixel we iterate over the mask
                for maskj in range(n):
                    ...
                    if (-maski+i) >= 0 and (maski+i) < x and (maskj+j) < y and(-maskj+j) >= 0: # bounding the pixels to be considered (edge cases)

                        Weight = Weight+mask[maski][maskj]
                        r, g, b = src.get(YY, XX)
                        avg = (r+g+b)//3
                        avg = min(max(0, avg), 255)
